...
class Bonus2Test extends KernelTestCase
{

    public function testLevelScript()
    {
        echo "\n* MLS: Testing Manhunt 1 PC (compile Bonus2) ==> ";

        $resources = new Resources();
        $resources->workDirectory = explode("/tests/", __DIR__)[0] . "/tests/Resources";

        $resource = $resources->load('/Archive/Mls/Manhunt1/PC/bonus2.mls', MHT::GAME_MANHUNT, MHT::PLATFORM_PC);
        $handler = $resource->getHandler();

        $mhls = $handler->unpack( $resource->getInput(), MHT::GAME_MANHUNT, MHT::PLATFORM_PC);

        $levelScriptCompiler = new \App\Service\CompilerV2\Compiler($mhls[0]['SRCE'], MHT::GAME_MANHUNT, MHT::PLATFORM_PC);
        ...
        for($i = 0; $i < count($mhls) ; $i ++){
            ...
            $compiler = new \App\Service\CompilerV2\Compiler($testScript['SRCE'], MHT::GAME_MANHUNT, MHT::PLATFORM_PC);
