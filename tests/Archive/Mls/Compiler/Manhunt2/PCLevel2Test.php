...
class PCLevel2Test extends KernelTestCase
{
    ...
    public function testLevelScript()
    {
        ...
        $mhls = $resources->load('/Archive/Mls/Manhunt2/PC/A02_The_Old_House.mls')->getContent();
        ...
        if ($mhls[0]['CODE'] != $levelScriptCompiled['CODE']){
            foreach ($mhls[0]['CODE'] as $index => $item) {
                if ($levelScriptCompiled['CODE'][$index] == $item){
                    ...
                    echo "MISMATCH need |" . $item . "| got |" . $levelScriptCompiled['CODE'][$index] . "|\n";
                }
            }
            exit;
        }
//var_dump($levelScriptCompiled['STAB']);
//        exit;
